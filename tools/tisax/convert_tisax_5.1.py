"""
Simple script to convert TISAX v5.1 excel in a Auditrex Excel file
...
parser = argparse.ArgumentParser(
    ...
    description="convert TISAX controls offical v5.1 Excel file to Auditrex Excel file",
)
    ...
    if title in ("Information Security", "Prototype Protection", "Data protection"):
        for row in tab:
            ...
            if control_number and re.fullmatch(r"\d", control_number):
                ...
                output_table.append(
                    ("", 1, control_number, control_question.strip(), "", "")
                )
            if control_number and re.fullmatch(r"\d\.\d+", control_number):
                level = 3
                print(control_number, control_question)
                output_table.append(
                    ("", 2, control_number, "", control_question.strip(), "")
                )
                if req_must:
                    output_table.append(
                        ("x", 3, "", "(must)", req_must.strip(), "must")
                    )
            if control_number and re.fullmatch(r"\d\.\d+\.\d+", control_number):
                ...
                output_table.append(
                    ("", level, control_number, "", control_question.strip(), "")
                )
                output_table.append(
                    ("x", level + 1, "", "(must)", req_must.strip(), "must")
                )
