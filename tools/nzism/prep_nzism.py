"""
simple script to transform the official NZISM XML file to an Excel file for Auditrex framework conversion tool
"""
...
def process_paragraph(level, paragraph):
    ...
    if paragraph.has_attr("compliance"):
        ...
        implementation_group = paragraph_compliance.upper()

    if paragraph.has_attr("classification"):
        paragraph_classification = paragraph["classification"]
        if len(paragraph_compliance) > 0:
            groups = []
            items = paragraph_classification.split(", ")
            for item in items:
                groups.append(f"{item} - {paragraph_compliance.upper()}")
...
parser = argparse.ArgumentParser(
    prog="convert_nzism",
    description="convert NZISM's XML file to Auditrex Excel file",
)
...
for chapter in nzism.find_all("chapter"):
    ...
    output_table.append(("", 1, chapter_id, chapter_name))
    ...
    for section in chapter.findChildren("section", recursive=False):
        ...
        for subsection in section.findChildren("subsection", recursive=False):
            ...
            for block in subsection.findChildren("block", recursive=False):
                ...
                output_table.append(("", 4, "", "", block_name, ""))
...
implementation_compliance_groups = ["MUST", "MUST NOT", "SHOULD", "SHOULD NOT"]
ws3.append(["ref_id", "name", "description"])
for implementation_classification_group in implementation_classification_groups:
    for implementation_compliance_group in implementation_compliance_groups:
        text = (
            f"{implementation_classification_group} - {implementation_compliance_group}"
        )
        ws3.append(
            [
                text,
