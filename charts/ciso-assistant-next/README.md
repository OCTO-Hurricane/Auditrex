...
A Helm chart for Auditrex k8s's deployment
...
| global.image.imagePullPolicy | string | `"IfNotPresent"` | If defined, a imagePullPolicy applied to all Auditrex deployments |
| global.image.registry | string | `"ghcr.io"` | If defined, a registry applied to all Auditrex deployments |
| global.image.tag | string | `""` | Overrides the global Auditrex image tag whose default is the chart appVersion |
...
| ingress.enabled | bool | `true` | Enable an ingress resource for the Auditrex |
...
| ingress.path | string | `"/"` | The path to Auditrex |
